...
class AddressService {
  ...
  Future<Address?> addAddress(Map<String, dynamic> data) async {
    try {
      final response = await ApiClient.dio.post('/addresses', data: data);
  ...
  Future<bool> updateAddress(String id, Map<String, dynamic> data) async {
    try {
      final response = await ApiClient.dio.patch('/addresses/$id', data: data);
  ...
  Future<bool> deleteAddress(String id) async {
    try {
      final response = await ApiClient.dio.delete('/addresses/$id');
      return response.statusCode == 200;
    } catch (e) {
